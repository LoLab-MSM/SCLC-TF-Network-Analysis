% Prepare cytoscape plotting files for some MDSTs
clc
clear
...
indOfSelectedEdges = find(nets_MDST(networkID,:) == 1);

selectedEdges = SCLCNetEdges(indOfSelectedEdges,:);

[adj_MST_soln, adj_G_soln] = MST(selectedEdges);

edgeStatus = [listOfEdges(:,1:2), zeros(size(listOfEdges,1),1)];
...
filename = sprintf('AnExampleFoundMDST_netID%d.xlsx',networkID);
...
p_eg = plotOptimalTree([nets_MDST(networkID,:),10], SCLCNetEdges, true);
