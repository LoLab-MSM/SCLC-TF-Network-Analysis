...
% 1. Load dataset
    ...
    SCLCNetEdges = SCLCNetEdges(idx,:); % eliminate the edges with prob 0.
...
% -------------------------------------------------------------------------
    ...
    [uniqueSols, finalObjValue] = GAoptimization_multiObj(listOfEdges, wantDense, isObjFunctMinimized, populationSize, useParallel);
...
% -------------------------------------------------------------------------
    ...
    if(wantDense)
        csvwrite('The_minimum_dense_spanning_trees.csv', uniqueSols)
    else
        csvwrite('The_minimum_sparse_spanning_trees.csv', uniqueSols)
    end
toc