function [networkStates, trackReaction, transitionStatus] = booleabayesSCLC(numIter, initialState, rules, attractors)
...
%         end
        ...
        if((isequal(networkStates(1:27,i),attractors(1,:)')) | (isequal(networkStates(1:27,i), attractors(2,:)')))
            %message = 'Transition occured from Non-NE to NE subtype!';
            transitionStatus = 1;
            break
        elseif((isequal(networkStates(1:27,i), attractors(3,:)')) | (isequal(networkStates(1:27,i), attractors(4,:)')))
            %message = 'Transition occured from Non-NE to NEv1 subtype!';
            transitionStatus = 2;
            break
        elseif((isequal(networkStates(1:27,i), attractors(5,:)')) | (isequal(networkStates(1:27,i), attractors(6,:)')) | (isequal(networkStates(1:27,i), attractors(7,:)')) | (isequal(networkStates(1:27,i), attractors(8,:)')))
            %message = 'Transition occured from Non-NE to NEv2 subtype!';
            transitionStatus = 3;
            %networkStates(1:27,i);
            break
        end

    end
    
    s = size(trackReaction,2);
    networkStates = networkStates';
    networkStates = networkStates(1:s+1,1:27);
